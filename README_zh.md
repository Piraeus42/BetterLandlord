...
- **游戏性能优化** — 复用选卡卡面、减少 resolver 临时分配，并缓存销毁类型计数，降低游玩过程中的重复开销
- **动画速度总控** — 一项选项一键统一调节游戏内全部动画速度

### 游戏 Bug 修复

- 修复鸽子保护相邻符号后不成长的问题
- 修复迈达斯炸弹摧毁通配符时不产生金币的问题
- 修正简体中文 Fine Print 37 文本（原文误用了 Fine Print 18 的断头台文本）
- 修复复制 Seed 按钮在剪贴板被其他程序占用时偶发报错弹窗的问题
- 修复启动游戏前复制的内容无法粘贴到种子输入框的问题（Ctrl+V 现在始终能粘贴你最近复制的内容）
- 修复游戏内过大数值结算时可能溢出异常的问题
- 修复脏话罐 / 猪猪存钱罐零收益结算可能导致 Boss 战卡死的问题
...
感谢卡瓦尼幸运房东交流群（QQ: 262548401）的社区测试者和 @无名萌新。

---

> **合规与公平性**
>
> 本 Mod **不包含**、不修改、不重新分发任何游戏原始源代码或美术资产。
> 时间线查看器中的图标以嵌入式程序资源编译——不会读取、提取或重新打包游戏文件。
>
> **自定义种子对局已自动从原生统计和 Steam 成就中排除。**
> 此排除在引擎层面强制实施——种子局不会增加游戏局数、胜/败记录，也不会解锁成就。
> Mod 自身的胜率统计同样过滤种子局。
