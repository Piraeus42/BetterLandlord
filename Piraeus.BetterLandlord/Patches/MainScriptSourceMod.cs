...
# Cold-boot recovery uses append-only JSONL.  The cursor makes each native
# save serialize only new events instead of the complete run history.
...
func _bh_start_run():
    ...
    if _bh_run_id != '':
        ...
        var _old_json = 'user://betterHistory/events_' + _bh_run_id + '.json'
        var _old_jsonl = 'user://betterHistory/events_' + _bh_run_id + '.jsonl'
        if _d2.file_exists(_old_json):
...
#
# Native save_game() can be called multiple times during one popup resolution.
# The former implementation JSON.print()'d the entire event history every time,
# which made pick-symbol latency grow with play time.  JSONL lets us append only
# the events that were created since the last successful native save.

func _bh_persist_events_incremental(force_snapshot = false):
    var _result = {
        'mode': 'noop',
        'event_count': 0,
        'serialized_events': 0,
    ...
    var _must_snapshot = force_snapshot or _bh_events_snapshot_required or _bh_events_persisted_count > _bh_events.size()
    if not _must_snapshot and (not _bh_events_dirty or _bh_events_persisted_count >= _bh_events.size()):
        return _result

    var _d = Directory.new()
    if not _d.dir_exists('user://betterHistory'):
    ...
    _bh_events_dirty = false
    _bh_events_snapshot_required = false
    _result['mode'] = 'snapshot' if _must_snapshot else 'append'
    _result['event_count'] = _bh_events.size()
    _result['serialized_events'] = _serialized
    _result['bytes'] = _lines.length()
    return _result

# Compatibility entry point retained for existing patches.  It no longer emits
# a complete JSON array on every save; the work is bounded by new event count.
func _bh_dump_raw_events():
    return _bh_persist_events_incremental(false)

# Finalization paths may remove stale run_end events.  Rewrite only there so a
# forced close after returning to title still restores an exact event sequence.
func _bh_force_persist_raw_events():
    return _bh_persist_events_incremental(true)

func _bh_load_events_from_jsonl(path):
    var _loaded = []
    var _file = File.new()
    if not _file.file_exists(path) or _file.open(path, File.READ) != OK:
        return _loaded
    while not _file.eof_reached():
        var _line = _file.get_line().strip_edges()
        if _line == '':
            continue
        var _parsed = JSON.parse(_line)
        if _parsed.error == OK and typeof(_parsed.result) == TYPE_DICTIONARY:
            _loaded.append(_parsed.result)
    _file.close()
    return _loaded

# Load events from the JSONL sidecar and truncate at the native save point.
# Legacy .json array sidecars remain readable for already-started runs.
func _bh_load_events_for_continue(save_spins):
    # Warm Continue ? events are still in memory, no need to reload from disk.
    if _bh_events.size() > 0:
        return
    var _base = 'user://betterHistory/events_' + _bh_run_id
    var _jsonl_path = _base + '.jsonl'
    var _legacy_path = _base + '.json'
    var _loaded = _bh_load_events_from_jsonl(_jsonl_path)
    if _loaded.size() == 0:
        var _legacy_file = File.new()
        if not _legacy_file.file_exists(_legacy_path) or _legacy_file.open(_legacy_path, File.READ) != OK:
            return
        var _legacy_text = _legacy_file.get_as_text()
        _legacy_file.close()
        var _legacy_parsed = JSON.parse(_legacy_text)
        if _legacy_parsed.error != OK or typeof(_legacy_parsed.result) != TYPE_ARRAY:
            return
        _loaded = _legacy_parsed.result

    _bh_events.clear()
    var _keep = true
    for _ev in _loaded:
        if typeof(_ev) != TYPE_DICTIONARY:
            if _keep:
                _bh_events.append(_ev)
            continue
        if _keep:
            var _pl = _ev.get('payload', {})
            if typeof(_pl) == TYPE_DICTIONARY:
                var _sn = _pl.get('spin_num', null)
                if _sn != null and (typeof(_sn) == TYPE_INT or typeof(_sn) == TYPE_REAL):
                    if int(_sn) > save_spins:
                        _keep = false
                        continue
            _bh_events.append(_ev)

    # The sidecar can contain events produced after the native save point.  The
    # next save rewrites the JSONL once, then appends normally again.
    _bh_events_persisted_count = _bh_events.size()
    _bh_events_dirty = true
    _bh_events_snapshot_required = true
