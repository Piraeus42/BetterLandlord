@echo off
setlocal

REM ================================================
REM  SlotWeave Mod Build Script
REM  修改 GAME_DIR 为你的游戏安装目录
REM ================================================

set GAME_DIR=D:\steam\steamapps\common\Luck be a Landlord
set MOD_ID=MyMod

REM 设置 GDWEAVE_CORE 供 csproj 中的 HintPath 使用
set GDWEAVE_CORE=%GAME_DIR%\SlotWeave\core

echo.
echo Building %MOD_ID%...
dotnet build "%~dp0..\Mod" -c Release
if %ERRORLEVEL% NEQ 0 (
    echo BUILD FAILED
    pause
    exit /b %ERRORLEVEL%
)

echo.
echo Deploying to game mods folder...
set MOD_DIR=%GAME_DIR%\SlotWeave\mods\%MOD_ID%
if not exist "%MOD_DIR%" mkdir "%MOD_DIR%"
copy /Y "%~dp0..\Mod\bin\Release\net8.0\*.dll"   "%MOD_DIR%\" >nul
copy /Y "%~dp0..\manifest.json"                    "%MOD_DIR%\" >nul

echo Done — %MOD_DIR%
echo.
echo Run the game to test.
