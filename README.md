...
2. `GDWeave/core/` → 复制到游戏根目录
3. Mod → `GDWeave/mods/<ModId>/`（每个 Mod 一个文件夹，内含 `manifest.json` + `.dll`）
...
```bash
dotnet build -c Debug -p:GDWeavePath="D:\steam\steamapps\common\Luck be a Landlord"
```
